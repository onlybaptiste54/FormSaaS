...
export default function NewCampaignPage() {
  ...
  async function create(e: FormEvent) { e.preventDefault(); if (prompt.trim().length < 10) return; setBusy(true); setError(""); try { const c = await api<Campaign>("/campaigns", { method: "POST", body: JSON.stringify({ prompt }) }); router.push(`/campaigns/${c.id}?tab=form`); } catch (err) { setError(err instanceof Error ? err.message : "Création impossible"); setBusy(false); } }
