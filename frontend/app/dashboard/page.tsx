...
import { ArrowRight, FileCheck2, FileText, MousePointerClick, Plus, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => { Promise.all([api<Stats>("/stats"), api<Campaign[]>("/campaigns")]).then(([s, c]) => { setStats(s); setCampaigns(c.slice(0, 3)); }); }, []);
  return <Shell><div className="page-wrap dashboard-page"><PageHeader eyebrow="MARDI 25 AOÛT" title="Bonjour Camille." description="Voici ce qui s’est passé depuis votre dernière visite." actions={<Link href="/campaigns/new" className="button button-primary"><Plus size={18} />Créer une campagne</Link>} />
    {!stats ? <Loading /> : <>
      <section className="metric-grid">
        ...
        <Metric icon={<TrendingUp />} label="Cette semaine" value={stats.week_responses.toString()} note="Nouvelles réponses" positive />
      </section>
